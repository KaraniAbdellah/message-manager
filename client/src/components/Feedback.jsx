...
export default function Feedback() {
    ...
    useEffect(() => {
        document.addEventListener("click", function(event) {
            if (event.target.classList.contains("delete")) {
                const idOfRemovedEle = (event.target.parentElement.parentElement.getAttribute("id")); 
                console.log(idOfRemovedEle);
                event.target.parentElement.parentElement.remove();
                

                axios.delete(`http://127.0.0.1:3001/deleteUser/${idOfRemovedEle}`).then(async (res) => {
                    console.log(res);
                });
            }
        });
    ...
    return (
        <div className='feedback p-10 bg-zinc-800 rounded-sm w-[100%] lg:w-[80%] md:w-[100%] sm:w-[100%]'>
            <div className="inputs flex justify-between items-center border-solid border-b-yellow-500 border-zinc-800 border-2">
                        ...
                        <tbody  className="bg-white">
                            {
                                filtredUsers.map((user) => {
                                    return(
                                        <tr key={user._id} id={user._id} className="hover:bg-gray-100">
